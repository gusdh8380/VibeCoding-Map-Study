---
description: VibeMap 학습 세션 시작 (2–3개 노드 심화 + 퀴즈). 인자 없으면 이어서, `<id>`면 특정 노드, `next`면 다음 노드.
argument-hint: "[node-id | next]"
---

VibeMap 학습 세션을 진행한다. `vibetutor` 스킬을 사용하고 `tutor/TUTOR.md`의 세션 흐름(2절)을 그대로 따른다.
