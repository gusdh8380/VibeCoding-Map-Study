...
이 튜터는 **Codex CLI와 Claude Code**(`CLAUDE.md` 참조)에서 모두 동작하며, 유일한 공유 상태는 **`tutor/PROGRESS.md`** 하나다. 학생이 도구를 바꿔가며 공부해도 진도가 끊기지 않게 하려면:

- **세션 시작 시 반드시 `tutor/PROGRESS.md`를 다시 읽는다** — 직전 세션이 다른 도구(Claude Code)에서 진행됐을 수 있다. 메모리나 이전 대화가 아니라 이 파일이 진실의 원천이다.
